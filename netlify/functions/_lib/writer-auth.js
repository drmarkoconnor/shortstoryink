...
const COOKIE_NAME = 'ss_writer_access'
...
export function renderWriterPage({
	title,
	heading,
	nav = [],
	content,
	...
	const navHtml = nav.length
		? `<nav class="ww-nav" aria-label="Writer navigation">${nav
	...
	<head>
		<meta charset="utf-8"/>
		<title>${escHtml(title || 'Writer')}</title>
		<style>
			...
			p{margin:.45rem 0}
			a{color:#0f172a}
			.ww-nav{display:flex;flex-wrap:wrap;gap:.65rem;margin:.45rem 0 1rem 0;font:14px/1.4 system-ui,sans-serif}
			.ww-nav a{text-decoration:none;border-bottom:1px solid #cbd5e1}
			.ww-panel{margin:1rem 0 0;padding:.95rem 1rem;border:1px solid #e5e7eb;border-radius:10px;background:#fcfcfc}
			.ww-muted{color:#6b7280}
			.ww-msg{padding:.6rem .75rem;border-radius:8px;margin:.8rem 0 1rem;font:14px/1.4 system-ui,sans-serif}
			.ww-msg--ok{background:#edf7ed;color:#1f5130}
			.ww-msg--warn{background:#fff7ed;color:#7c2d12}
			.ww-msg--err{background:#fdecec;color:#7a1f1f}
			...
			.card{padding:.85rem .9rem;border:1px solid #e5e7eb;border-radius:10px;background:#fff}
			${extraStyles || ''}
		</style>
	</head>
	<body>
		<h1>${escHtml(heading || title || 'Writer')}</h1>
		${navHtml}
		${content}
	</body>
	</html>`
...
export function writerAuthCookie(token) {
...
export function clearWriterAuthCookie() {
	return `${COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0`
}

export function extractWriterAccessToken(event) {
...
export async function ensureWriterProfile(user) {
	if (!user?.id) return
	const payload = {
		id: user.id,
		role: 'writer',
	...
	await fetch(`${SUPABASE_URL}/rest/v1/profiles?on_conflict=id&select=id`, {
		method: 'POST',
		headers: {
...
export async function requireWriter(event) {
	...
	const token = extractWriterAccessToken(event)
	if (!token)
	...
	await ensureWriterProfile(user)
	return { user, token }
}

export async function sbServer(path) {
